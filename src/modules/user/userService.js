...
const { validateOrgMembershipChange } = require("./policies/membershipGovernance");
...
async function updateUserService(user_id, updateFields) {
  ...
  if (updateFields.fullname) {
    if (
      typeof updateFields.fullname !== "string" ||
      !updateFields.fullname.trim()
    ) {
...
const removeUserFromOrgService = async ({
  actorUserId,
  actorRole,
  targetUserId,
  org_id,
}) => {
  const targetUser = await validateOrgMembershipChange({
    actorRole,
    actorUserId,
    targetUserId,
    org_id,
...
module.exports = {
  ...
  updateUserRoleService: withLogging(
    updateUserRoleService,
    "updateUserRoleService",
  ),
  removeUserFromOrgService: withLogging(
